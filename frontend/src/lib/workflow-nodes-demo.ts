/**
 * Legacy U04 demo helpers — retained VERBATIM so the old demo
 * `WorkflowEditor.svelte` keeps compiling after U05 moved the single taxonomy
 * to `workflow-nodes.ts` and the conversion/YAML layer to `workflow-flow.ts`.
 *
 * Nothing new should import from here: the demo editor is orphaned on disk
 * (the page route renders `FlowEditor` since U05) and this module only exists
 * to keep the legacy component's imports valid. Remove together with
 * `WorkflowEditor.svelte` when the demo shell is deleted.
 */
