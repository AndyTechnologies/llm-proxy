...
The editor MUST expose the full shared taxonomy of 14 node types — start, end, llm_call (in generate/refine/passthrough modes), condition, loop, fan, join, pipeline, rag_local, data.code, memory, embeddings, router, output — each draggable onto the canvas. The palette SHALL be derived from the engine's node-type registry so the editor and engine cannot drift.

#### Scenario: Palette completeness

- GIVEN the editor open
- WHEN the palette is inspected
- THEN all 14 node types are listed, each draggable onto the canvas
...
- GIVEN a llm_call node without a model
