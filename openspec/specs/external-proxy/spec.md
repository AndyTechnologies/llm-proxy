...
Expose the gateway as an OpenAI-compatible API on localhost port 4317: /v1/chat/completions, /v1/completions, /v1/models (and /v1/embeddings), routing to the four provider kinds (local, OpenAI-compatible, Anthropic, OpenRouter) with optional keychain-backed auth.
...
The proxy SHALL route requests across the four provider kinds (local, OpenAI-compatible, Anthropic, OpenRouter) according to model mapping.
