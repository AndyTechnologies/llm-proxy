...
- [ ] 4.1 `package.json` (edit): `build:binary` + `--asset dist/ui`.
- [ ] 4.2 Bundle guard: gzip prod app JS ≤ 100 KB, recorded via `scripts/measure-bundle.ts`.
- [ ] 4.3 MAJOR-3 `scripts/binary-smoke.ts`: build from unrelated cwd; assert embedded `/ui` (read-only) + `/ui/assets/*` (read-only), disk fallback, `UI_DIR` override.
