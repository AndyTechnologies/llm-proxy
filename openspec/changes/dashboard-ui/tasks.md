...
- [x] 3.6 Modify `src/server.ts`: add `/api/ui/*`(read-only) + `/ui`(read-only) branches; auth guard on `/api/ui/*`(read-only)+SSE when `BEARER_TOKEN` set, `/ui`(read-only) open (dashboard-api Req auth)
- [x] 3.7 Retry: manual retry only failed `llm_call`, max 1/step, non-streaming, result stored; RED already-retried refused + non-llm_call refused (dashboard-api Req retry)

## Phase 4 — Slice D: Static SPA (/ui)

- [x] 4.1 Create `src/ui/index.html`: ARIA landmarks, keyboard nav, WCAG AA, `<dialog>`, EventSource client
